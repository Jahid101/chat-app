...
export function normalizeConversation(raw: any): Conversation {
    return {
        ...
        name: pick(raw, ['name', 'title'], 'Conversation'),
        isGroup: Boolean(raw?.isGroup ?? raw?.is_group),
        participants: (raw?.participants ?? raw?.members ?? []).map(normalizeUser),
...
async function request<T>(path: string, options: RequestInit = {}, token?: string): Promise<T> {
    ...
    if (!res.ok) throw new ApiError((await res.text()) || `Request failed (${res.status})`, res.status);
